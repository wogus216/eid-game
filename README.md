...
효과음은 음원 파일 없이 코드로 합성합니다(WebAudio). 외부 요청도, 로딩도 없습니다.

- 브라우저 자동재생 정책 때문에 **첫 키 입력 전에는 소리가 나지 않습니다.** 행사 전
  아무 키나 한 번 눌러 해제해 두세요.
- **노트북 스피커가 아니라 행사장 음향에 연결**해야 100명이 듣습니다. 3.5mm/HDMI 출력
  연결 후 시스템 출력 장치가 맞는지 확인하세요.
- 소리를 빼고 진행하려면 `M`으로 음소거하면 됩니다(좌하단에 표시됨).

### 본행사 안전장치

- **화면 꺼짐 방지**: 앱이 켜져 있는 동안 Wake Lock으로 절전을 막습니다. 다만
  브라우저가 거부하는 경우가 있으니 **노트북 전원 연결 + 시스템 절전/화면보호기
  끄기**를 함께 해두세요(이중 안전장치).
- **실수 이탈 방지**: 게임 진행 중 새로고침·창 닫기를 시도하면 확인창이 뜹니다.
  대기 화면에서는 잃을 진행이 없으므로 뜨지 않습니다.
